...
RUN brew update && \
    ...
    brew install python@3.11 gzip p7zip lhasa bash curl gnu-tar openssl unzip gpatch dos2unix perl bc autoconf automake libtool pkg-config bison node git make nasm cmake ninja mingw-w64 && \
    brew cleanup -s && \
    rm -rf $(brew --cache)
