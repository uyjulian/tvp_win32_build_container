...
RUN brew update && \
    brew upgrade && \
    brew install bash curl gnu-tar gzip bzip2 xz zstd p7zip openssl lhasa unzip gpatch dos2unix perl bc autoconf automake libtool bison node git make nasm cmake ninja mingw-w64 && \
    brew cleanup -s && \
    rm -rf $(brew --cache)
